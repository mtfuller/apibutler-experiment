...
export class GenerateModelFilesAction extends ChatGptAction {
    ...
    protected generatePrompt(state: AgentState): BasePrompt {
        const systemMessage = generateSystemMessage(
            `Act as a AI agent that is able to problem solve and generate code to create business solution. You will be given a description of a new software solution ` + 
            `that a product team wants to implement. Your job is to generate a set of JSON ` + 
            `objects that each represent different entities needed to meet the requirements. ` + 
